...
class Lion(Animal):
    def intro(self):
        print(f"The {self.name} is great a soaking up the sun.")

class Zebra(Animal):
    def intro(self):
         print(f"The {self.name} loves running with his friends.")

class Elephant(Animal):
    def intro(self):
         print(f"The {self.name} can even paint a picture with his trunk!")

Simba = Lion("lion")
Dumbo = Elephant("elephant")
Marty = Zebra("zebra")

print(f"We have so many animals at the zoo, three whole animals--if you can believe it! Simba, a {Simba.name}, Dumbo, an {Dumbo.name}, and Marty, a {Marty.name}. They all have different hobbies.")
Simba.intro() 
Marty.intro() 
Dumbo.intro()
